...
export default function transformRpMessage(str, color) {
  ...
  var renderTag = {
    'ACTION': {
      open(color) {
        var contrast = color ? getContrast(color) : 'auto';
        return '<span class="message-star-tag" style="background-color:'+color+'; color:'+contrast+'">*';
      },
      close: '*</span>'
    },
    'BOLD': { open() { return '<b>' }, close: '</b>' },
    'ITC': { open() { return '<i>' }, close: '</i>' },
    'BOLDITC': { open() { return '<b><i>' }, close: '</i></b>' },
    'STRIKE': { open() { return '<del>' }, close: '</del>' },
  };
